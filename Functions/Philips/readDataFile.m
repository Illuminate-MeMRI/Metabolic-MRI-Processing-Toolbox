function [data, list] = readDataFile(listpath, varargin)
% [data, list] = readDataFile(listpath, varargin)
%
% Reads the raw CSI-data data-file and store all data into an array. Data
% from .list file is used to index each voxel. Due large data-files, data
% is read in chunks.
% 
% Input:          none      (-)      - Opens up .data-file selection GUI
%                 path      (string) - Path to .list or .data-file, assumes
%                                      same file-name for these files.
%                 list-data (struct) - Data structure of list-file
%                                      as returned from readListFile.
%       Varargin:
%                 save_data (int)    - Save loaded data to matfile (0,
%                                      default or 1).
%
...
%
% Known issue: list-struct from readListFile not useable to load data
%              again....Due splitting of noise in list.data.
%
% Quincy van Houtum. v08.2025
...
% ID for a 31P RX 16ch array and body coil for receive.
...
fprintf('#FIDS: %i | #Samples: %i  \n', size(ldat,1), ndimf);
fprintf('Dimensions loaded: \n'); fprintf('| %4s ', loi{loi_max_one==0}); 
fprintf('\nMax size: \n'); fprintf('| %4i ', loi_max); fprintf('\n');

% Display run-time
t1 = toc; 
fprintf('Processed .data-file using .list-file. Elapsed: %4.2fs \n', t1);
...
% set in code outside of try-block: data-struct and list-struct. 
    ...
    fperror = strcat('mrs_readDataFile_errorlog_',...
                      string(datetime('now', 'Format','HHmmss')), '.mat');
    save(fperror,'err');
    warning('Raw data loaded. Indexing failed. See error log.');
end
