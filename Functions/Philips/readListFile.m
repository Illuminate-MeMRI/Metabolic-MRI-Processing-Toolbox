function list = readListFile(fpn_list)
% [list] = readListFile(fpn_list)
%
...
if fid_r == -1
    warning('readListFile:Wrong_Path',...
...
while ~feof(fid_r)                                                           
    ...
    if ~strcmp(tline_split(1), '#') && ~strcmp(tline_split(1), '.')         % If line starts with hash (35) or dot (46) else data line.
        ...
        if length(tline_split)<2
            warning(['readListFile:corrupt list-file. ' ,...
...
catch, warning('readListFile:char_2_double_error', ...
